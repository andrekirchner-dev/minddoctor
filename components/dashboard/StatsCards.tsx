  ...
  const cards = [
    {
      ...
      value: stats.loading ? "—" : String(stats.hoje),
    ...
    {
      title: "Esta Semana",
      value: stats.loading ? "—" : String(stats.semana),
          ...
          >
            ...
            <div className="h-1.5 bg-muted rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width: stats.loading ? "0%" : stats.total === 0 ? "0%" : `${Math.min(100, (parseInt(c.value) / Math.max(stats.mes, 1)) * 100)}%`,
                  background: c.color,
                }}
              />
            </div>
            <p className="text-[10px] text-muted-foreground mt-1.5">
              {stats.loading ? "Carregando..." : stats.mes > 0 ? `${stats.mes} este mês` : "Nenhuma consulta este mês"}
            </p>
          </div>
        );
      })}
